...
- **One share you open is now the default, instead of one share per folder.**
  Sharing each top folder separately saves a tap on a phone, but it costs you
  everywhere else: there is no single thing to pin to Quick Access or map a drive
  to, saved shortcuts and bookmarks break whenever the folder list changes, and a
  top folder named like one of Windows' own shares (`Users`) collides with it.
  Everything now lives under one `\\server\FamilyNAS` again. Both layouts are still
  offered on the SMB tab, with the trade-off spelled out rather than implied.
